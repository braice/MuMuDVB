 ...
 */
int autoconf_finish_full(mumudvb_chan_and_pids_t *chan_and_pids, autoconf_parameters_t *autoconf_vars, multicast_parameters_t *multicast_vars, tuning_parameters_t *tune_p, fds_t *fds, unicast_parameters_t *unicast_vars, int server_id, void *scam_vars)
{
	...
	chan_and_pids->number_of_channels=autoconf_services_to_channels(autoconf_vars, chan_and_pids->channels, multicast_vars->common_port, tune_p->card, tune_p->tuner, unicast_vars, multicast_vars, server_id, scam_vars); //Convert the list of services into channels
...
int autoconf_new_packet(int pid, unsigned char *ts_packet, autoconf_parameters_t *autoconf_vars, fds_t *fds, mumudvb_chan_and_pids_t *chan_and_pids, tuning_parameters_t *tune_p, multicast_parameters_t *multicast_vars,  unicast_parameters_t *unicast_vars, int server_id, void *scam_vars)
{
		...
		if(pid==0) //PAT : contains the services identifiers and the PMT PID for each service
		{
			if(get_ts_packet(ts_packet,autoconf_vars->autoconf_temp_pat))
			{
				if(autoconf_read_pat(autoconf_vars))
				{
					...
					autoconf_finish_full(chan_and_pids, autoconf_vars, multicast_vars, tune_p, fds, unicast_vars, server_id, scam_vars);
				}
			}
		}
			...
			if((!chan_and_pids->channels[curr_channel].autoconfigurated) &&(chan_and_pids->channels[curr_channel].pmt_pid==pid)&& pid)
			{
				if(get_ts_packet(ts_packet,chan_and_pids->channels[curr_channel].pmt_packet))
				{
					...
					if(autoconf_read_pmt(chan_and_pids->channels[curr_channel].pmt_packet, &chan_and_pids->channels[curr_channel], tune_p->card_dev_path, tune_p->tuner, chan_and_pids->asked_pid, chan_and_pids->number_chan_asked_pid, fds)==0)
					{
						...
						if(autoconf_vars->autoconfiguration!=AUTOCONF_MODE_PIDS)
						{
							autoconf_set_channel_filt(tune_p->card_dev_path, tune_p->tuner, chan_and_pids, fds);
							//We free autoconf memory
							autoconf_freeing(autoconf_vars);
 ...
 * if it's finished, we open the new descriptors and add the new filters
 */
int autoconf_poll(long now, autoconf_parameters_t *autoconf_vars, mumudvb_chan_and_pids_t *chan_and_pids, tuning_parameters_t *tune_p, multicast_parameters_t *multicast_vars, fds_t *fds, unicast_parameters_t *unicast_vars, int server_id, void *scam_vars)
{
	int iRet=0;
	...
	else if (now-autoconf_vars->time_start_autoconfiguration>AUTOCONFIGURE_TIME)
	{
		...
		else if(autoconf_vars->autoconfiguration==AUTOCONF_MODE_FULL)
		{
			...
			autoconf_vars->time_start_autoconfiguration=now;
			iRet = autoconf_finish_full(chan_and_pids, autoconf_vars, multicast_vars, tune_p, fds, unicast_vars, server_id, scam_vars);
		}

  ...
  while(!scam_params->getcwthread_shutdown) {
    ...
    if (*request == CA_SET_PID)
    {
      ...
      if(scam_params->ca_pid.index == -1) {
        ...
        for (curr_channel = 0; curr_channel < chan_p->number_of_channels; curr_channel++) {
          mumudvb_channel_t *channel = &chan_p->channels[curr_channel];
          for (curr_pid = 1; curr_pid < channel->pid_i.num_pids; curr_pid++) {
            if ((channel->pid_i.pids_type[curr_pid] != PID_EXTRA_VBIDATA) && (channel->pid_i.pids_type[curr_pid] != PID_EXTRA_VBITELETEXT) && (channel->pid_i.pids_type[curr_pid] != PID_EXTRA_TELETEXT) && (channel->pid_i.pids_type[curr_pid] != PID_EXTRA_SUBTITLE) && (channel->pid_i.pids[curr_pid] == (int) scam_params->ca_pid.pid)) {
              pthread_mutex_lock(&channel->cw_lock);

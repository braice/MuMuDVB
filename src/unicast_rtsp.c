...
int unicast_send_rtsp_setup (unicast_client_t *client, int CSeq, int Tsprt_type);
int unicast_send_rtsp_teardown (int Socket, int CSeq, unicast_client_t *client , mumudvb_channel_t *channels, unicast_parameters_t *unicast_vars, fds_t *fds, int number_of_channels);
...
int unicast_handle_rtsp_message(unicast_parameters_t *unicast_vars, unicast_client_t *client, mumudvb_channel_t *channels, int number_of_channels, fds_t *fds)
{
  ...
  */
  ...
  {
    unicast_send_rtsp_describe(client->Socket, CSeq);  // send response trough RTSP Server socket
    unicast_flush_client(client);
    return 0;
  }
  else if(strstr(client->buffer,"OPTIONS")==client->buffer)
  {
    unicast_send_rtsp_options(client->Socket, CSeq);  // send response trough RTSP Server socket
    unicast_flush_client(client);
    return 0;
  }
  ...
  {
    iRet=unicast_send_rtsp_play(client->Socket, CSeq, client, channels, number_of_channels);  // send response trough RTSP Server socket
    if(iRet)
      return CLOSE_CONNECTION;
    unicast_flush_client(client);

/** 
    log_message(MSG_DETAIL,"Unicast : We delete the client %s:%d, socket %d\n",inet_ntoa(client->Socket.sin_addr), ntohs(client->Socket.sin_port), client->Socket);
    if (client->Socket >= 0)
    {
...
/** @todo important : test if the client already exist when a new client arrives, delete dead clients*/
  ...
  {
    unicast_send_rtsp_teardown(client->Socket, CSeq, client, channels, unicast_vars, fds, number_of_channels);
    return CLOSE_CONNECTION;
  }
  else
  {
...
int unicast_send_rtsp_setup (unicast_client_t *client, int CSeq, int Tsprt_type)
{
  int Socket=client->Socket;	// On ne recupere pas la socket en parametre mais depuis la structure "client"
...
int unicast_send_rtsp_teardown (int Socket, int CSeq, unicast_client_t *client , mumudvb_channel_t *channels, unicast_parameters_t *unicast_vars, fds_t *fds, int number_of_channels)
{
  ...
  {
    log_message(MSG_INFO,"Unicast : Malformed TEARDOWN request, we return 404\n");
    err404=1;
  }
  if(!requested_channel)
    err404=1;
  ...
  if(requested_channel)
  {
     log_message(MSG_INFO,"Unicast : TEARDOWN channel %d\n",requested_channel);
    // TODO Supprimer l'IP du client dans la liste des IP unicast.
    //client->channel=requested_channel-1;
    log_message(MSG_INFO,"Unicast : On supprime la chaine %d de la liste de diffusion de la socket\n", requested_channel-1);

    log_message(MSG_INFO,"Unicast : Valeur dans le tableau channels de la socket %d\n", &channels[requested_channel-1]);

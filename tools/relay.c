...
#define PACKET_SIZE 188
  ...
  //seven MPEG2 TS paquets in one UDP PAQUET
#define MTU PACKET_SIZE*7
...
main (int argc, char **argv)
{
  ...
  fprintf (stderr, "relay\n Program used to Relay an UDP multicast stream to an UDP unicast (or multicast) stream\n");
  ...
  if (argc != 5)
  ...
  else
    {
      ...
      portIn = atoi(argv[2]);
      strncpy (ipOut, argv[3],MAX_IP_LENGTH);
      portOut = atoi(argv[4]);
    }


  ttl = 2;
  fprintf (stderr, "Streaming. ...\nA dot is printed every 500 datagrams\n");
  
  /* Init udp */
  socketOut = makesocket (ipOut, portOut, ttl, &sOut); //le makeclientsocket est pour joindre automatiquement le flux
  ...
  while (!Interrupted)
      ...
      if (lengthPacket != MTU)
	{
      ...
      sendudp(socketOut, &sOut, temp_buf,MTU);
      if(num%500==0)
	{
	  num=0;
	  num80++;
